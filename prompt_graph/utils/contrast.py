...
def contrastive_loss(x1, x2, temperature=0.1):
    r"""Realizes contrastive loss and generate model output and graph data
    after random perturbation.


    Args:
        x1,x2 (Tensor): The input feature.
        temperature (float): Adjusts the sensitivity of similarity (default: :obj:`0.1`).
    """
...
def generate_random_model_output(data, model):
    r"""Generates the output of the model after random perturbation.

    Args:
        data (Data): The feature of graphs.
        model (model): Used as the basis for creating a modified version (vice_model) by perturbing its parameters.
    """
...
def generate_corrupted_graph(graph_data, aug='dropE', aug_ratio=0.1):
    r"""Generates the graph data with random disturbance according to different augmentation methods.

        Args:
            graph_data (Data): The feature of graphs.
            aug (str): The method of augmentation.
...
def generate_corrupted_graph_via_drop_node(data, aug_ratio=0.1):
    r"""The perturbed graph data is generated by deleting nodes.
...
def generate_corrupted_graph_via_drop_edge(data, aug_ratio):
    r"""The perturbed graph data is generated by deleting edges.
...
def genereate_corrupted_graph_via_shuffle_X(data):
    r"""
        Perturb one graph by row-wise shuffling X (node features)
        without changing the A (adjacency matrix).
    """
